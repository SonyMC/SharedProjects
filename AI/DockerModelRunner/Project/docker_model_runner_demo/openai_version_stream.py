...
# Print the model's reply (streaming response)
print("Response: ", end="", flush=True)  # Print header without newline
for chunk in response:                    # Iterate over streaming chunks
    if chunk.choices[0].delta.content is not None:  # Check if chunk has content
        print(chunk.choices[0].delta.content, end="", flush=True)  # Print content without newline
